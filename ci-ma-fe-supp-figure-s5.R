...
ci_ma_fe_simul <- function(Nsim, dim, true_bet, Num_ext, num_study, study_size, A, mu_y, sigma2_y, df_y, mu_x, sigma2_x, x_corr, num_boot, seed_num){
  ...
  error_count <- rep(0,Nsim)
  error_count_exact <- rep(0,Nsim)
  ...
  for(i in 1:Nsim){
    ...
    for(bb in 1:num_boot){
      ...
      if (singular_flag_exact) {
        error_count_exact[i] <- error_count_exact[i] + 1 
        next
      }
    ...
    #  Var_bet_rootn_boot <- Var_bet_rootn_boot/(count-1)
  ...
  error_indices <- which(error_count > 0)
  error_values <- error_count[error_count > 0]
  
  error_summary_table <- rbind(
    error_sim_index = error_indices,
    error_counts = error_values
  )
  
  error_indices_exact <- which(error_count_exact > 0)
  error_values_exact <- error_count_exact[error_count_exact > 0]
  
  error_summary_table_exact <- rbind(
    error_sim_index_exact = error_indices_exact,
    error_counts_exact = error_values_exact
  )
  ...
  total_output<- list(output_table,output_table_exact,hat_bet, error_summary_table, error_summary_table_exact)
  names(total_output) <- c("Real", "Oracle", "Estimates", "Error-summary", "Error-summary-exact")
...
pdf_filename_panel1 <- paste0("panel1-k10-p8.pdf")
...
pdf_filename_panel2 <- paste0("panel2-k10-p8.pdf")
...
pdf_filename_panel3 <- paste0("panel3-k10-p8.pdf")
...
pdf_filename_panel4 <- paste0("panel4-k10-p8.pdf")
